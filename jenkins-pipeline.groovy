...
podTemplate(
  ...
  node('myJenkinsMavenSlave') {
    ...
    stage('Analise de codigo') {
     echo "Code Analysis"
     sh "${mvnCmd} sonar:sonar -Dsonar.host.url=http://sonarqube.cicd-tools.svc.cluster.local:9000/ -Dsonar.projectName=rhforum"
    }
    ...
    stage('Publicando no Nexus') {
      ...
      sh "${mvnCmd} deploy -DskipTests=true -DaltDeploymentRepository=nexus::default::http://nexus.cicd-tools.svc.cluster.local:8081/repository/maven-releases"
    }
    ...
    stage('Build Imagem Openshift') {
      ...
      sh "oc policy add-role-to-user edit system:serviceaccount:cicd-tools:jenkins -n rhforum-app-dev"
